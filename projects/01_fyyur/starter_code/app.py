...
class Venue(db.Model):
    ...
    #one venue contain many artists and one venue has many shows
    #one artist has many venues and one venue has many artists
...
class Show(db.Model):
    ...
    #this table is created due to many to many relationships
    #one artist has many venues and one venue has many artists
    #one artist has many shows and one venue has many artists
    #one artist has many shows and one venue has many artists
...
def venues():
  ...
  #       num_upcoming_shows should be aggregated based on number of upcoming shows per venue.
    #query to return venues order by state and city name
    venues = Venue.query.order_by(Venue.state, Venue.city).all()

    data = []
    #dictionary to store venue temporary then append to data
    dict_to_store_venue = {}
    prev_city = None
    prev_state = None
    #loop for venue getting
    for venue in venues:
        venue_data = {
            'id': venue.id,
            'name': venue.name,
        ...
        # if venue is already in state, a 
        if venue.city == prev_city and venue.state == prev_state:
            dict_to_store_venue['venues'].append(venue_data)
        else:
            if prev_city is not None:
                data.append(dict_to_store_venue)
            dict_to_store_venue['city'] = venue.city
            dict_to_store_venue['state'] = venue.state
            dict_to_store_venue['venues'] = [venue_data]
...
def search_venues():
  ...
  # search for "Music" should return "The Musical Hop" and "Park Square Live Music & Coffee"
    ...
    venues = Venue.query.filter(
        #ilike work as sql like query %any number of alphabtes {} specific words
        Venue.name.ilike('%{}%'.format(search_term))).all()

    data = []
    for venue in venues:
        dict_to_store_venue = {}
        dict_to_store_venue['id'] = venue.id
        dict_to_store_venue['name'] = venue.name
        dict_to_store_venue['num_upcoming_shows'] = len(venue.shows)
        data.append(dict_to_store_venue)
